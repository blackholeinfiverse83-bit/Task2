"""
Prosody Mapper Service (Portable Version)
...
def _get_prosody_mappings_path() -> Path:
    ...
    return Path(__file__).parent / "data" / "prosody_mappings.json"
...
def _load_prosody_mappings() -> Dict[str, Any]:
    ...
    if not mappings_path.exists():
        # Fallback to local data folder if running from root
        mappings_path = Path("data/prosody_mappings.json")
        
    if not mappings_path.exists():
        raise FileNotFoundError(
            f"Prosody mappings file not found at {mappings_path.absolute()}"
        )
