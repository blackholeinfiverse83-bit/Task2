import { NextRequest, NextResponse } from 'next/server'

const AUTH_BASE_URL =
  process.env.NEXT_PUBLIC_AUTH_API_URL || 'https://ai-being-ecwj.onrender.com'

/**
 * Proxy POST /api/auth/login → external microservice
 * Adapts the microservice response ({ token, user }) to the legacy shape
 * the frontend context expects ({ success: true, data: { token, user } }).
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()

    const res = await fetch(`${AUTH_BASE_URL}/api/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    })

    const data = await res.json()

    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: data.message || data.error || 'Invalid email or password' },
        { status: res.status }
      )
    }

    return NextResponse.json({
      success: true,
      data: {
        token: data.token,
        user: data.user,
      },
    })
  } catch (error) {
    console.error('Login proxy error:', error)
    return NextResponse.json(
      { success: false, error: 'Authentication failed' },
      { status: 500 }
    )
  }
}
